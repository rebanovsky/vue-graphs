<template>
  <div
    class="playground-component w-[100%] flex gap-[8px] rounded-[8px]"
    style="height: calc(100vh - 169px)"
  >
    <div class="calendar-side gridlines w-[320px] rounded-[8px] p-[8px]">CALENDAR SIDE</div>
    <div class="calendar-main flex flex-col w-[100%] flex-1 gap-[8px]">
      <div class="calendar-main-top w-[100%] gridlines rounded-[8px] h-[100%] p-[8px]"></div>
      <div class="calendar-main-bottom gridlines rounded-[8px] h-[100%] p-[8px]"></div>
    </div>
  </div>
</template>
...
export default {
  components: { TheHeader, TheList, SideFeed },
  setup() {
    const stocks = [
      {
        name: "Apple Inc.",
    ...
    const formattedData = line1.map((item) => ({
      x: item.Date,
      y: item.Close,
    }));
    ...
    return { settings, widget, stocks, news, chartData };
  },
};
</script>
